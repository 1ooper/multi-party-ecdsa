...
fn distributed_keygen (store_number: usize) {
    ...
    let uuid = format!("keygen_{}", store_number);
    //signup:
    ...
    .unwrap();
    let path = format!("keystore/keystore_{}_{}.json", party_num_int, store_number);
    fs::write(path,keygen_json).expect("Unable to save !");
}
fn main() {

   let threads_num = env::args().nth(2).unwrap().parse::<usize>().unwrap();
   use std::thread;
   for i in 0..threads_num {
       println!("No.{} thread starts",i);
       thread::spawn(move || 
        {
            distributed_keygen(i);
        }
    );
   }
