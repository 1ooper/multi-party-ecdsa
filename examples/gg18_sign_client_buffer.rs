...
fn distributed_sign(message_str: String, keystore: String) {
...
fn main() {
    ...
    let tx_buffer = HashMap::new();
    let db_mtx =RwLock::new(tx_buffer);
    {   
        ...
        let mut tx_buffer = db_mtx.write().unwrap();
        for _i in 0..sign_number {
            
            let keystore = "keys1.store".to_string();
            tx_buffer.insert(message_str.clone(), keystore);
        }
    }
    rocket::ignite()
        .mount("/", routes![act_tx])
...
#[post("/acttx", format="json", data = "<tx>")]
fn act_tx(db_mtx: State<RwLock<HashMap<String, String>>>, tx: Json<String>) -> Json<Result<String, ()>>{
    let act_tx = tx.into_inner();
    let tx_buffer = db_mtx.read().unwrap();
    let keystore = tx_buffer.get(&act_tx).unwrap();
    distributed_sign(act_tx, keystore.to_string());
